...
/*
// This has to be updated to the latest API
...
public:
    CalcPressure(
        ...
        int item3 = H2D_FN_VAL,
        int item4 = H2D_FN_VAL
        )
...
int main(int argc, char* argv[])
{
  ...
  LinearProblem sys(&wf);
  sys.set_spaces(Tuple<Space*>(&s0, &s1, &s3, &s4));
  int ndof = get_num_dofs(Tuple<Space*>(&s0, &s1, &s3, &s4));
  ...
  */
  ...
  for (int i = 1; i <= num_time_steps; i++)
  {
    ...
    sys.assemble(mat, rhs);
    //p.push("sys", LinSystem_from_C(&sys));
    p.exec("import util");
    //p.exec("util.run(sys)");
    if (!solver->solve(mat, rhs)) error ("Matrix solver failed.\n");
    w0_sln.set_coeff_vector(&s0, rhs);
    w1_sln.set_coeff_vector(&s1, rhs);
    w3_sln.set_coeff_vector(&s3, rhs);
    w4_sln.set_coeff_vector(&s4, rhs);
    //sys.solve(Tuple<Solution*>(&w0_sln, &w1_sln, &w3_sln, &w4_sln));
    //error("stop");

    // visualization
    sprintf(title, "Current density, time %g", TIME);
    w13_view.set_title(title);
    w13_view.show(&w1_prev, &w3_prev, H2D_EPS_LOW);
